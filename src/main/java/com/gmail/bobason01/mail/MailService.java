...
public class MailService {
    ...
    private static final long SESSION_TIMEOUT = 300000L;
    ...
    public static void send(Player sender, Plugin plugin) {
        ...
        String lang = LangManager.getLanguage(senderId);
        MailSession session = sessions.get(senderId);
        if (session == null || session.items == null || session.items.isEmpty() || session.target == null) {
    ...
    public static long getExpireTime(UUID playerId) {
        ...
        return buildExpireTime(LocalDateTime.now(), time).atZone(TimeZone.getDefault().toZoneId()).toInstant().toEpochMilli();
    }
    ...
    private static LocalDateTime buildExpireTime(LocalDateTime base, Map<String, Integer> timeData) {
        if (timeData == null || timeData.isEmpty()) return base.plusYears(100);
        return base.plusYears(timeData.getOrDefault("year", 0))
    ...
    private static List<ItemStack> cloneValidItems(List<ItemStack> items) {
        ...
        if (items != null) {
            for (ItemStack item : items) {
                if (item != null && item.getType() != Material.AIR) clones.add(item.clone());
            }
        }
        return clones;
    }
